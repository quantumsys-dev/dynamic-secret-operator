...
Write-Step "Triggering Valid TLS Certificate Rotation via ESO for '$Domain'..."
...
Write-Step "Observing Synchronized Ingestion: Key Vault -> ESO -> DSO -> Canary -> Production"

Write-Host @"

Watching DSO Policy status (Press Ctrl+C to stop watching):
Command: kubectl get dynamicsecretpolicy eso-ingress-tls-policy -n dso-examples -w

Watching ExternalSecret sync:
Command: kubectl get externalsecret ingress-tls-cert-eso -n dso-examples -w
...
Write-Host "✅ Valid ESO TLS Certificate Rotation successfully tested!" -ForegroundColor Green
