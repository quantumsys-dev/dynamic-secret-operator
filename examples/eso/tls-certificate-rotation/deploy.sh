...
echo "=================================================================="
echo "🚀 Deploying ESO + DSO TLS Certificate Rotation Example to AKS..."
...
echo "registrar (GoDaddy, Namecheap, Cloudflare, Registro.br, Hostinger, etc.)"
...
if [ "${NS_DELEGATED}" != "true" ]; then
    ...
    echo "   ./deploy.sh -d \"${DOMAIN}\" -k \"${KEYVAULT_NAME}\""
    echo "=================================================================="
    exit 1
fi

# 6. Create or verify certificate in Azure Key Vault
...
CLIENT_ID="$(az identity show -n "id-dso-dev" -g "${RESOURCE_GROUP}" --query "clientId" -o tsv 2>/dev/null || true)"
if [ -n "${CLIENT_ID}" ]; then
    echo "✅ Using Managed Identity 'id-dso-dev' (Client ID: ${CLIENT_ID})"
    if ! az identity federated-credential show --name "dso-eso-sa-federation" --identity-name "id-dso-dev" -g "${RESOURCE_GROUP}" >/dev/null 2>&1; then
        ISSUER_URL="$(az aks list -g "${RESOURCE_GROUP}" --query "[0].oidcIssuerProfile.issuerUrl" -o tsv 2>/dev/null || true)"
        if [ -n "${ISSUER_URL}" ]; then
            echo "ℹ️  Federating Managed Identity with ServiceAccount 'dso-examples:eso-azure-sa'..."
            az identity federated-credential create \
                --name "dso-eso-sa-federation" \
                --identity-name "id-dso-dev" \
                --resource-group "${RESOURCE_GROUP}" \
                --issuer "${ISSUER_URL}" \
                --subject "system:serviceaccount:dso-examples:eso-azure-sa" \
                --audience "api://AzureADTokenExchange" \
                --output none 2>/dev/null || true
            echo "✅ Workload Identity federation established for ESO."
        fi
    fi
fi

# 7. Apply bootstrap secret in cluster
...
# 8. Apply manifests
echo "📄 Deploying ESO SecretStore, ExternalSecret, Nginx Gateway, and DSO Policy..."
TENANT_ID="$(az account show --query tenantId -o tsv 2>/dev/null || true)"
sed -e "s/\${KEYVAULT_NAME}/${KEYVAULT_NAME}/g" \
    -e "s/\${DOMAIN}/${DOMAIN}/g" \
    -e "s/\${AZURE_CLIENT_ID}/${CLIENT_ID}/g" \
    -e "s/\${AZURE_TENANT_ID}/${TENANT_ID}/g" \
    "${SCRIPT_DIR}/manifests.yaml" | kubectl apply -f - || {
...
echo "=================================================================="
echo "✅ ESO TLS Certificate Rotation Example deployed successfully on AKS!"
echo "=================================================================="
echo ""
