...
Write-Step "Simulating Invalid/Expired Certificate with ESO Mode..."
Write-Info "Target Domain:     $Domain"
Write-Info "Target Key Vault:  $KeyVaultName"

# 1. Capture current production revision
$initialPolicyJson = kubectl get dynamicsecretpolicy eso-ingress-tls-policy -n dso-examples -o json 2>$null
$initialRevision = ""
...
# 2. Generate expired certificate bundle
Write-Step "Generating synthetically expired certificate bundle..."
...
# 3. Inject expired certificate bundle into Key Vault secret
...
Write-Step "Monitoring ESO Sync & DSO Circuit Breaker on AKS..."
Write-Info "Expected Behavior:"
Write-Info "1. ESO detects updated secret in Key Vault and reconciles 'ingress-tls-cert-synced'."
Write-Info "2. DSO detects secret update via watch label 'dso.quantumsys.dev/managed: watch'."
Write-Info "3. DSO launches isolated canary pod (tls-gateway-canary)."
Write-Info "4. Synthetic TLS Probe detects certificate expiration and fails."
Write-Info "5. Failures reach threshold (3), tripping Circuit Breaker."
Write-Info "6. Ephemeral canary is deleted; production remains 100% HEALTHY on revision $initialRevision!"
...
while ($elapsed -lt $timeout) {
    $policyJson = kubectl get dynamicsecretpolicy eso-ingress-tls-policy -n dso-examples -o json 2>$null
    if ($policyJson) {
