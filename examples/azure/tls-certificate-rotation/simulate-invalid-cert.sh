...
echo "=================================================================="
echo "⚠️  Simulating Invalid Certificate Injection for '${DOMAIN}'..."
echo "🔑 Target Key Vault: ${KEYVAULT_NAME}"
echo "=================================================================="

# 1. Capture current production revision
CURRENT_REV="$(kubectl get dynamicsecretpolicy aks-ingress-tls-policy -n dso-examples -o jsonpath='{.status.currentRevision}' 2>/dev/null || true)"
...
echo "⏳ Monitoring DSO Circuit Breaker on AKS..."
TIMEOUT=90
ELAPSED=0
TRIPPED=false

while [ $ELAPSED -lt $TIMEOUT ]; do
    FAIL_COUNT="$(kubectl get dynamicsecretpolicy aks-ingress-tls-policy -n dso-examples -o jsonpath='{.status.consecutiveFailures}' 2>/dev/null || echo "0")"
    IS_TRIPPED="$(kubectl get dynamicsecretpolicy aks-ingress-tls-policy -n dso-examples -o jsonpath='{.status.conditions[?(@.type=="CircuitBreakerTripped")].status}' 2>/dev/null || echo "False")"
    ...
    if [ "${IS_TRIPPED}" = "True" ]; then
        ...
        MSG="$(kubectl get dynamicsecretpolicy aks-ingress-tls-policy -n dso-examples -o jsonpath='{.status.conditions[?(@.type=="CircuitBreakerTripped")].message}' 2>/dev/null || true)"
